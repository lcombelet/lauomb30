...
$username = $password = $confirm_password = $activation_code = $registration_code = $email = "";
$username_err = $password_err = $confirm_password_err = $registration_code_err = $email_err = "";

// Processing form data when form is submitted
if(isset($_POST['submit'])) {
	unset($_POST['submit']);

	// Validate regitration code
	if(empty(trim($_POST["registration_code"]))){
		$registration_code_err = "Please enter your registration code.";
	} elseif(trim($_POST["registration_code"]) != md5('LAUOMB')){
		$registration_code_err = "Invalid registration code.";
	}
  ...
  if(empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($registration_code_err) && empty($email_err)){
